...
As of this writing: **13 suites / 198 tests**, all passing, and every suite imports the real
...
```bash
node tools/run-uat-automated-tests.mjs
```

This regenerates `Documentations/AutoTest Results/automated-test-run.json` (structured) and
`automated-test-run-log.txt` (full Jest output). `Automated_UAT_Test_Run_Report.docx` in the same
folder is the narrative writeup built from that JSON — it's authored separately, not
auto-generated by the script, so re-run the script and then refresh the docx by hand (or ask
whoever/whatever last built it to regenerate it) after any test-suite change so neither goes
stale.
